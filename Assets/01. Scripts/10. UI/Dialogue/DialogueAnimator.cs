...
public class DialogueAnimator 
{
    ...
    public DialogueAnimator(TMP_Text _textBox, AudioSourceGroup _audioSourceGroup)
    {
        textBox = _textBox;
    ...
    {
        textAnimating = true;
        float secondsPerCharacter = 1f / 30f;
        ...
        while (true)
        {
            ...
            for (int j = 0; j < charCount; j++)
            {
                ...
                if (charInfo.isVisible) //Invisible characters have a vertexIndex of 0 because they have no vertices and so they should be ignored to avoid messing up the first character in the string whic also has a vertexIndex of 0
                {
                    ...
                    {
                        float timeSinceAnimStart = Time.unscaledTime * 1.15f - charAnimStartTime;
    ...
    private void PlayDialogueSound(AudioClip voice_sound)
    {
        ...
        {
            timeUntilNextDialogueSound = UnityEngine.Random.Range(0.05f, 0.07f);
